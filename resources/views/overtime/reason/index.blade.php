...
<div class="col-md-12">
	...
	<div class="portlet box green">
		<div class="portlet-title">
			<div class="caption">
				<i class="fa fa-comments"></i>List of Reason
			</div>
			<div class="actions">
<!-- 				saveIggBut -->
				<div class="btn-group btn-group-devided" data-toggle="modal" data-target="#newReason" accessKey="N">
		...
		<div class="portlet-body" style="display: block;">
			<div class="table-scrollable">
				<table class="table table-striped table-hover">
					...
					<tbody>
						...
						<tr id="TableClassRowID-{{$item->id}}">
							<td>{{$item['id']}}</td>
							<td>{{$item['reason_subject']}}</td>
							<td>{{$item['created_at']}}</td>
							<td>{{$item['updated_at']}}</td>
							<td>
								<button type="button" onclick="reasonEdit({{$item['id']}})" accesskey="E" class="btn yellow-gold">
									<i class="fa fa-pencil"></i> Ed<u>i</u>t
								</button>
								<button type="button" onclick="reasonDel({{$item['id']}})" accesskey="D" class="btn red-mint">
				...
				<?php echo $reasonList->render();?>
			</div>
		</div>
	</div>
</div>
<script src="{{asset('/js/overtime/reason/index.js')}}"></script>
@include('overtime.modal.newReason') 
@endsection
