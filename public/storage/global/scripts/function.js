...
function format(icon) {
    ...
    return '<i class="glyphicon glyphicon-' + $(originalOption).data('icon') + '"></i> ' + icon.text;
}
      ...
      success: function(data,response){//it's always good to check server output when developing...
        ...
        if (action){
            ...
            if(IsPaid==0){//No Payment Leave Allow any days
              ...
              $("#eLeaveTotalDaysToApply").html(n);
              $("#_LeaveTotalDaysToApply").val(n);
              bal=StringToFloat($("#eLeaveBalance").html())-StringToFloat($("#eLeaveTotalDaysToApply").html());
            }
...
function CancelLeave(){
   ...
   // Allow only to Cancel When the Start Date greater than the current date
   if((FormatDate(sdate)<FormatDate(CurrentDate)) && LeaveStatus==3 && HRFullAccess!=1){
...
function ExecuteCreditLeaveNow(){
    ...
    var ApplicantID=$("#LeaveApplicant").select2('val');
    var RecipientID=$("#LeaveRecipient").select2('val');
      ...
      success: function(data,response){//it's always good to check server output when developing...
        ...
        if (action){
            ...
            if (parseInt(arr[0])>0) {
               ...
               var DateRange="to a pending/approved leave request from "+FormatDate($('#LeaveStartDate').jqxDateTimeInput('getDate'));
               ...
               mAlert.Show('AlertDiv', msg, "The System has Detected a conflict of leave date<br>"+DateRange);
               HideProgressBar();
            } else{
               if(leaveIsSubmit==1){//If Submitted then Redirect to dashboard
                  document.location="dashboard.php";
               }else{
                  document.location="leave.php?id="+arr[1];
               }
            }
        }
     } 
...
function AcknowledgeCancelLeaveRequest(){
    MessageBoxEx("Are you sure you want to approve this Cancel Leave Request?",BootstrapDialog.TYPE_WARNING,"ExecuteCancelLeaveRequest","HideProgressBar");
}
...
function ApproveRejectLeaveRequest(IsReject){
   ...
   if(IsReject==0){
      ...
      msg="Are you sure you want to Approved this Leave Request?";
      func="ApproveLeaveRequest";
      BT=BootstrapDialog.TYPE_INFO;
   }
   //if(nComment.length<=0 && IsReject==0){
   //   MessageBoxAlert("Comment is Required on <strong>Leave Request Rejection!</strong>",BootstrapDialog.TYPE_DANGER);
   //   HideProgressBar();
   //}else{
